    ...
    >
      <el-table-column label="Name" min-width="300px" align="center">
        <template slot-scope="{ row }">
          ...
          <router-link :to="{ name: 'Graph', params:{ predicate: row.name}}">{{ row.name }}</router-link>
        </template>
      </el-table-column>
...
import { searchPredicate } from '@/api/esSearch'
...
export default {
  ...
  methods: {
    getList() {
      ...
      searchPredicate(data, this.base.page, this.base.limit).then(response => {
    ...
    //   this.$axios
    //     .post(
    //       'http://localhost:8899/search/predicate/' +
    //         this.base.page +
    //         '/' +
    //         this.base.limit,
    //       {
